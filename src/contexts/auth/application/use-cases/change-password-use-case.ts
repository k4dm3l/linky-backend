...
} from "@/contexts/auth/application/commands/change-password-command";
import {
  AuthResult,
  AuthResultBuilder,
...
export class ChangePasswordUseCase {
  ...
  ): Promise<AuthResult<ChangePasswordCommandResult>> {
    let transaction: any;

    try {
      this.logger.info("Changing password for user", {
      ...
      if (error instanceof Error) {
        return AuthResultBuilder.error(error.message, "PASSWORD_CHANGE_ERROR", {
